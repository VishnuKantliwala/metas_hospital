...
$con->connectdb();
    ...
    {	    
	  //print_r($_POST);die;
        ...
        {
            if ( $_FILES["download_file"]['type'] != "application/pdf" && $_FILES["download_file"]['type'] !="application/vnd.openxmlformats-officedocument.presentationml.presentation" && $_FILES["download_file"]['type'] != "application/vnd.ms-powerpoint") 
            {
                $error = "Please select a PDF file or a PPT file";
            }
            else
            {
                //-----
                @unlink("../health_talk_pdf/". $frontimgpdf2);
                if($_FILES["download_file"]['type'] =="application/vnd.openxmlformats-officedocument.presentationml.presentation" || $_FILES["download_file"]['type'] == "application/vnd.ms-powerpoint")
                {
                    $pdf_file = createPPT('download_file',"../health_talk_pdf/");
                }
                else
                {
                    $pdf_file = createPDF('download_file',"../health_talk_pdf/");
                }
		...
		$con->insertdb("UPDATE `tbl_pdf` SET pdf_name='".$pdf_name."', `description`='".$description."' ".$sqlFile." where pdf_id = 3");
				
				
			
		// header("location:health_talk.php");
    ...
    if(isset($_POST['btnDeletePdf']))
    {
        $frontimgpdf2=$_POST['frontimgpdf2'];
        @unlink("../health_talk_pdf/". $frontimgpdf2);   
        $con->insertdb("UPDATE `tbl_pdf` SET pdf_file='' where pdf_id = 3");	
                ...
                <div class="container-fluid">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="card-box">
                                <?
                                $sqlPdf = $cn->selectdb("select * from tbl_pdf where pdf_id = 3");
                                ...
                                ?>
                                <h4 class="mt-0 mb-2 header-title">Health Talk Form</h4>
                                <form action="" class="form-horizontal" method="post"  id="myform" name="myform" enctype="multipart/form-data">				
                                    ...
                                    <div class="form-group">
                                        <label for="inputEmail3" class="col-sm-2 control-label">PDF or PPT</label>
                                        <div class="col-sm-4">
